...
func find_or_create(id string) *SessionObject {
	...
	err := db.QueryRow(query, this.Id).Scan(&data, &this.Stamp)
	if err != nil {
		this.store()
	}
	err = deserialize(data, &this.Data)
	if err != nil {
...
func (this *SessionObject) store() {
	...
	_, err = db.Exec("REPLACE INTO `"+table+"`(id,data,stamp) VALUES(?,?,?)", this.Id, b, this.Stamp)
	if err != nil {
		this.err(err)
	}
}
